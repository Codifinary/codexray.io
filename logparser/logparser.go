...
func main() {
	...
	for {
		...
		ch <- LogEntry{Timestamp: time.Now(), Content: strings.TrimSuffix(line, "\n"), Level: LevelUnknown}
	}
	d := time.Since(t)
	defer parser.Stop()
...
func order(counters []LogCounter) {
...
func output(counters []LogCounter, screenWidth, maxLinesPerMessage int, duration time.Duration) {
...
func colorize(level Level, format string, a ...interface{}) string {
	c := "\033[37m" // grey
	switch level {
